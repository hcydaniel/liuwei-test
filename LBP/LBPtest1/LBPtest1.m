%原文：https://blog.csdn.net/lyqmath/article/details/6139043 
% By lyqmath
...
hold on;
[xt, yt] = meshgrid(round(linspace(1, size(I, 1), 30)), ...
round(linspace(1, size(I, 2), 40)));
mesh(yt, xt, zeros(size(xt)), 'FaceColor', ... 
'None', 'LineWidth', 1, ...
'EdgeColor', 'r'); % 分成40块，行   分成30块，列
subplot(2, 2, 3); imshow(BW);
title('二值图像', 'FontWeight', 'Bold');
[n1, n2] = size(BW);
r = floor(n1/10); % 分成 10块，行
c = floor(n2/10); % 分成 10块，列
x1 = 1; x2 = r; % 对应行初始化
s = r*c; % 块面积
for i = 1:10
y1 = 1; y2 = c; % 对应列初始化
